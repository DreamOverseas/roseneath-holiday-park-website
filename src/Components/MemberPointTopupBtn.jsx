...
const MemberPointTopupBtn = () => {
  ...
  const isCustomValid =
    /^\d{1,4}$/.test(customAmount) && Number(customAmount) > 0;
  ...
  const handleApprove = async () => {
    ...
    try {
      const searchRes = await axios.get(
        `${endpoint}/api/rhp-memberships?filters[Email][$eq]=${
  ...
  return (
    <>
      ...
      {showModal && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50'>
          <div className='bg-white rounded-lg p-6 w-full max-w-lg relative max-h-[70vh] overflow-y-auto'>
            <button
            ...
            <div className='flex justify-between mb-4 space-x-4'>
              <button
                className={`flex-1 h-24 border rounded-lg flex flex-col items-center justify-center ${
                  selectedAmount === 100 ? "!border-blue-600" : ""
                }`}
              ...
              <button
                className={`flex-1 h-24 border rounded-lg flex flex-col items-center justify-center ${
                  selectedAmount === 500 ? "!border-blue-600" : ""
                }`}
              ...
              <button
                className={`flex-1 h-24 border rounded-lg flex flex-col items-center justify-center ${
                  selectedAmount !== 100 && selectedAmount !== 500
                    ? "!border-blue-600"
                    : ""
            ...
            <p className='text-center mb-4'>
              ...
              <b className='text-green-800'>
                + {selectedBonus} {t("membership_discount")}
              </b>
            </p>
            ...
            <PayPalScriptProvider
              options={{ "client-id": paypalId, currency: "AUD" }}
            >
              <PayPalButtons
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50'>
          <div className='bg-white rounded-lg p-6 w-full max-w-sm text-center'>
            <h3 className='!text-amber-600'>
              {" "}
              <i class='bi bi-stars'></i> Yay! <i class='bi bi-stars'></i>{" "}
            </h3>
