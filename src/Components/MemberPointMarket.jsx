...
const MemberPointMarket = () => {
  ...
  const redeemProductDisplayName =
    i18n.language === "zh" ? redeemProduct?.Name : redeemProduct?.Name_en;
  ...
  const comfirmRedeemNow = async () => {
    ...
    try {
      const couponResponse = await fetch(
        `${couponSysEndpoint}/create-active-coupon`,
        {
          method: "POST",
      ...
      if (couponResponse.ok && couponData.couponStatus === "active") {
        ...
        const emailResponse = await fetch(
          `${emailApiEndpoint}/roseneathpark/coupon_distribute`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    <Container className='my-4'>
        ...
        <Modal show={showModal} onHide={handleModalClose}>
          ...
          <Modal.Body>
            {selectedProduct.Icon &&
              ...
              selectedProduct.Icon && (
                <img
                  src={`${import.meta.env.VITE_CMS_ENDPOINT}${
      ...
      {redeemProduct && (
        ...
        >
          ...
          <Modal.Body>
            <p>
              {t("membership_product")}
              {redeemProductDisplayName}
            </p>
            {(() => {
              ...
              return (
                <>
                  <p>
                    {t("membership_point")}：{cookiePoints} →{" "}
                    <b>{cookiePoints - redeemProduct.Price + currDeduction}</b>
                  </p>
                  <p>
                    {t("membership_discount")}：{cookieDiscountPoints} →{" "}
                    <b>{cookieDiscountPoints - currDeduction}</b>
                  </p>
                  <hr />
                  {maxDeduction > 0 ? (
                    <Form.Group>
                      <Row className='d-flex'>
                        <Col md={7}>
                          <Form.Label>
                            {t("membership_dis_p")} ({currDeduction}/
                            {maxDeduction})
                          </Form.Label>
                        </Col>
                        <Col md={5}>
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleDeductionChange(
                                    Math.min(maxDeduction, cookieDiscountPoints)
                                  )
                                }
                              >
                                Max
          ...
          <Modal.Footer>
            {(() => {
              ...
              const cookieDiscountPoint =
                JSON.parse(Cookies.get("user")).discount_p || 0;
              const sufficientPoints =
                cookiePoints >= redeemProduct.Price - currDeduction;
              ...
              return (
                ...
                >
                  {sufficientPoints && sufficientDiscountPoint
                    ? loadingRedeem
                      ? `${t("membership_redeeming")}`
                      : `${t("membership_redeem")}`
                    : sufficientPoints
                    ? `${t("membership_insuff_p")}`
                    : `${t("membership_insuff_d")}`}
                </Button>
              );
            })()}
          </Modal.Footer>
      ...
      {redeemProduct && showSuccessModal && (
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>{redeemProductDisplayName}</Modal.Title>
          </Modal.Header>
